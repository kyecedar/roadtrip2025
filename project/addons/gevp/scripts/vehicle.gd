...
#@export var front_ackermann : float = 0.15
...
## Surface detection uses node groups to identify the surface, so make sure
## your staticbodies and rigidbodies belong to one of these groups.
@export var rolling_resistance : Dictionary[String, float] = { "Road" : 1.0, "Dirt" : 2.0, "Grass" : 4.0 }
...
## Surface detection uses node groups to identify the surface, so make sure
## your staticbodies and rigidbodies belong to one of these groups.
@export var lateral_grip_assist : Dictionary[String, float] = { "Road" : 0.05, "Dirt" : 0.0, "Grass" : 0.0}
...
## Surface detection uses node groups to identify the surface, so make sure
## your staticbodies and rigidbodies belong to one of these groups.
@export var longitudinal_grip_ratio : Dictionary[String, float] = { "Road" : 0.5, "Dirt": 0.5, "Grass" : 0.5}
@export_subgroup("Front Axle", "front_")
...
const ANGULAR_VELOCITY_TO_RPM : float = 60.0 / TAU
...
var speed : float = 0.0
var motor_rpm : float = 0.0

var steering_amount : float = 0.0
var steering_exponent_amount : float = 0.0
var true_steering_amount : float = 0.0
var throttle_amount : float = 0.0
var brake_amount : float = 0.0
var clutch_amount : float = 0.0
var current_gear : float = 0
var requested_gear : float = 0
var torque_output : float = 0.0
var clutch_torque : float = 0.0
var max_clutch_torque : float = 0.0
var drive_axles_inertia : float = 0.0
var complete_shift_delta_time : float = 0.0
var last_shift_delta_time : float = 0.0
var average_drive_wheel_radius : float = 0.0
var current_torque_split : float = 0.0
var true_torque_split : float = 0.0
var brake_force : float = 0.0
var max_brake_force : float = 0.0
var handbrake_force : float = 0.0
var max_handbrake_force : float = 0.0
var is_braking : bool = false
var motor_is_redline : bool = false
var is_shifting : bool = false
var is_up_shifting : bool = false
var need_clutch : bool = false
var tcs_active : bool = false
var stability_active : bool = false
var stability_yaw_torque : float = 0.0
var stability_torque_vector : Vector3 = Vector3.ZERO
var front_axle_position : Vector3 = Vector3.ZERO
var rear_axle_position : Vector3 = Vector3.ZERO
...
class Axle:
	...
	var rotation_split : float = 0.5
	var applied_split : float = 0.5
	var torque_vectoring : float = 0.0
	var suspension_compression_left : float = 0.0
	var suspension_compression_right : float = 0.0
	var tire_size_correction : float = 0.0
	var differential_lock_torque : float = 0.0
...
func initialize():
	...
	mass = vehicle_mass
	var center_of_gravity : Vector3 = calculate_center_of_gravity(front_weight_distribution)
	...
	var max_tire_radius : float = maxf(front_tire_radius, rear_tire_radius)
	...
	var front_weight_per_wheel : float = vehicle_mass * front_weight_distribution * 4.9
	var front_spring_rate : float = calculate_spring_rate(front_weight_per_wheel, front_spring_length, front_resting_ratio)
	var front_damping_rate : float  = calculate_damping(front_weight_per_wheel, front_spring_rate, front_damping_ratio)
	...
	var rear_damping_rate : float = calculate_damping(rear_weight_per_wheel, rear_spring_rate, rear_damping_ratio)
	...
	var front_ackermann : float = (atan((wheel_base * tan(max_steering_angle)) / (wheel_base - (front_track_width * 0.5 * tan(max_steering_angle)))) / max_steering_angle) - 1.0
	var rear_track_width : float = rear_right_wheel.position.x - rear_left_wheel.position.x
	var rear_ackermann : float = (atan((wheel_base * tan(max_steering_angle)) / (wheel_base - (rear_track_width * 0.5 * tan(max_steering_angle)))) / max_steering_angle) - 1.0
	...
	if front_brake_bias < 0.0:
		var front_axle_spring_force : float = calculate_axle_spring_force(0.6, front_spring_length, front_spring_rate)
		var total_spring_froce : float = front_axle_spring_force + calculate_axle_spring_force(0.4, rear_spring_length, rear_spring_rate)
...
func _physics_process(delta : float) -> void:
	...
	if not vehicle_inertia:
		var rigidbody_inertia : Vector3 = PhysicsServer3D.body_get_direct_state(get_rid()).inverse_inertia.inverse()
...
func process_drag() -> void:
	var drag : float = 0.5 * air_density * pow(speed, 2.0) * frontal_area * coefficient_of_drag
	if drag > 0.0:
...
func process_steering(delta : float) -> void:
	var steer_assist_engaged : bool = false
	var steering_slip : float = get_max_steering_slip_angle()
	
	## Adjust steering speed based on vehicle speed and max steering angle
	var steer_speed_correction : float = steering_speed / (speed * steering_speed_decay) / max_steering_angle
	...
	var steer_correction : float = (1.0 - absf(steering_adjust)) * clampf(asin(local_velocity.normalized().x), -max_steering_angle, max_steering_angle) * countersteer_assist
	...
	var steer_correction_amount : float = 1.0
...
func process_throttle(delta : float) -> void:
	var throttle_delta : float = throttle_speed * delta
...
func process_clutch(delta : float):
	...
	var drive_inertia : float = motor_moment + (pow(absf(current_gear_ratio), 2.0) * gear_inertia) + drive_axles_inertia
	var drive_inertia_R : float = drive_inertia / (current_gear_ratio * current_gear_ratio)
	var reaction_torque : float = get_drive_wheels_reaction_torque() / current_gear_ratio
	var speed_difference : float = (motor_rpm / ANGULAR_VELOCITY_TO_RPM) - (get_drivetrain_spin() * current_gear_ratio)
	if speed_difference < 0.0:
		speed_difference = -sqrt(-speed_difference)
	var a : float = (motor_moment * drive_inertia_R * speed_difference) / delta
	var b : float = motor_moment * reaction_torque
	var c : float = drive_inertia_R * torque_output
	var clutch_factor : float = (1.0 - clutch_amount)
	var tcs_torque_reduction : float = 0.0
	...
	var clutch_reaction_torque : float = clutch_torque + tcs_torque_reduction
	var new_rpm : float = motor_rpm - ((ANGULAR_VELOCITY_TO_RPM * delta * clutch_reaction_torque) / motor_moment)
	if new_rpm < idle_rpm:
		new_rpm = idle_rpm
...
func process_transmission() -> void:
	...
	if automatic_transmission:
		...
		var ideal_wheel_spin : float = speed / average_drive_wheel_radius
		var drivetrain_spin : float = get_drivetrain_spin()
		var real_wheel_spin : float = drivetrain_spin * get_gear_ratio(current_gear)
		var current_ideal_gear_rpm : float = gear_ratios[current_gear - 1] * final_drive * ideal_wheel_spin * ANGULAR_VELOCITY_TO_RPM
		var current_real_gear_rpm : float = real_wheel_spin * ANGULAR_VELOCITY_TO_RPM
		
		if current_gear == -1:
			reversing = true
		
		if not reversing:
			var previous_gear_rpm : float = 0.0
			if current_gear - 1 > 0:
...
func process_drive(delta : float) -> void:
	...
	var drive_inertia : float = motor_moment + pow(current_gear_ratio, 2) * gear_inertia
	var is_slipping : bool = get_is_a_wheel_slipping()
	...
	var axle_a : Axle = front_axle
	var axle_b : Axle = rear_axle
	if true_torque_split <= 0.5:
		axle_a = rear_axle
		axle_b = front_axle
	var axle_difference : float = axle_a.get_spin() - axle_b.get_spin()
	...
	var transfer_torque : float = (a - b + c)/(axle_a.inertia + axle_b.inertia)
	transfer_torque = clampf(transfer_torque, -absf(drive_torque), absf(drive_torque)) * (1.0 - absf((0.5 - true_torque_split) * 2.0))
	var transfer_torque_2 : float = drive_torque - transfer_torque
...
func process_axle_drive(axle : Axle, torque : float, drive_inertia : float, delta : float) -> void:
	...
	if axle.is_drive_axle and axle.differential_lock_torque >= 0.0:
		if absf(torque) > axle.differential_lock_torque:
			...
			var couple_spin : float = axle.get_average_spin()
			...
			var left_reaction_torque_ratio : float = -absf((axle.wheels[0].get_reaction_torque()) / torque)
			var right_reaction_torque_ratio : float = absf((axle.wheels[1].get_reaction_torque()) / torque)
	...
	var rotation_sum : float = 0.0
	var split : float = (axle.rotation_split + 1.0) * 0.5
...
func process_stability() -> void:
	var is_stability_on : bool = false
	...
	if enable_stability:
		...
		var plane_xz : Vector2 = Vector2(local_velocity.x, local_velocity.z)
		if plane_xz.y < 0 and plane_xz.length() > 1.0:
			plane_xz = plane_xz.normalized()
			var yaw_angle : float = 1.0 - absf(plane_xz.dot(Vector2.UP))
...
func complete_shift() -> void:
	...
	if requested_gear < current_gear:
		var wheel_spin : float = speed / average_drive_wheel_radius
		var requested_gear_rpm : float = gear_ratios[requested_gear - 1] * final_drive * wheel_spin * ANGULAR_VELOCITY_TO_RPM
...
func calculate_brake_force() -> void:
	var friction : float = calculate_average_tire_friction(vehicle_mass * 9.8, "Road")
...
func calculate_spring_rate(weight : float, spring_length : float, resting_ratio : float) -> float:
	var corrected_resting_ratio : float = (spring_length * resting_ratio) / spring_length
	var target_compression : float = spring_length * corrected_resting_ratio * 1000.0
