...
#	if steering > 10:
#		Roadtrip.wheels.traction * .10
func process_go_go(delta: float) -> void:
	steering = move_toward(steering, Input.get_axis("turn_right", "turn_left") * deg_to_rad(Roadtrip.PLAYER_MAX_STEER), delta * Roadtrip.PLAYER_WHEEL_TURN_SPEED)
	var forwards_power : float = Input.get_action_strength("go_forwards") * Roadtrip.PLAYER_ENGINE_FORWARDS_POWER
	var backwards_power : float = -Input.get_action_strength("go_backwards") * Roadtrip.PLAYER_ENGINE_BACKWARDS_POWER
	
	# enforce max speed by cutting engine power if it goes above max speed.
	if miles_per_hour <= Roadtrip.PLAYER_MAX_SPEED:
...
func process_body(delta: float) -> void:
	# auto upright.
	rotation.z = move_toward(rotation.z, clamp(rotation.z, -deg_to_rad(Roadtrip.PLAYER_BODY_LEAN_HELP), deg_to_rad(Roadtrip.PLAYER_BODY_LEAN_HELP)), delta * 1.0)


func process_camera(delta: float) -> void:
	%CameraPivot.global_position = %CameraPivot.global_position.lerp(global_position, delta * Roadtrip.PLAYER_CAMERA_FOLLOW_AMOUNT)
	%CameraPivot.transform = %CameraPivot.transform.interpolate_with(transform, delta * Roadtrip.PLAYER_CAMERA_INTERPOLATE_AMOUNT)
	...
	forward_look_at = forward_look_at.lerp(global_position + Vector3(max(z_dot, Roadtrip.PLAYER_CAMERA_FORWARD_MIN), 0.0, max(z_dot, Roadtrip.PLAYER_CAMERA_FORWARD_MIN)) * transform.basis.z, delta * Roadtrip.PLAYER_CAMERA_INTERPOLATE_AMOUNT)
	
	%ForwardCamera.look_at(forward_look_at)
	
	# lean camera into turns.
	if Roadtrip.PLAYER_CAMERA_LEAN_RATIO:
		%ForwardCamera.rotate_z(x_dot / Roadtrip.PLAYER_CAMERA_LEAN_RATIO)

	
	%ForwardCamera.rotation.z = clamp(%ForwardCamera.rotation.z, -deg_to_rad(Roadtrip.PLAYER_CAMERA_MAX_ROLL_ROTATION), deg_to_rad(Roadtrip.PLAYER_CAMERA_MAX_ROLL_ROTATION))
